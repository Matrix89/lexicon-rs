...
use lexicon::lexicon::{Parameters, XrpcSubscription};
use proc_macro2::TokenStream;
use quote::{format_ident, quote};

use crate::CodeGen;

fn gen_body(namespace: &str, parameters: &Parameters) -> TokenStream {
    let url = format!(
...
impl CodeGen {
    pub fn gen_subscription(
        ...
        let desc = sub.description.unwrap_or("no desc".to_owned());
